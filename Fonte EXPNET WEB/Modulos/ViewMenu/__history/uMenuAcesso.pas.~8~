...
function DMAcesso: TDMAcesso;
begin
  Result := TDMAcesso(UniMainModule.GetModuleInstance(TDMAcesso));
end;

{ TdmAcessos }

procedure TDMAcesso.AbreTela(Sender, PageControl: TObject);
var
  vTag: Integer;
  vImageIndex: Integer;
  vNameForm: string;
...
begin
  ...
  vNameForm := StringReplace(vNameForm, 'act', 'TFrame', [rfIgnoreCase]);

    if vTag = 0 then
      TAbas.Nova(vNameForm, TUniPageControl(PageControl), vCaption, vImageIndex,
        True, True);
  if vTag = 10 then
    TAbas.Nova(vNameForm, TUniPageControl(PageControl), vCaption, vImageIndex,
      False, True);
