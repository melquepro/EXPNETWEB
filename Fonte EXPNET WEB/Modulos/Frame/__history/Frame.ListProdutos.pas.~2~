...
type
  TFrameListProdutos = class(TUniFrame)
...
procedure TFrameListProdutos.btnFiltroClick(Sender: TObject);
begin
PControl.NovaAba(TFrame(TFrameCadClientes),'Cadastro de Cliente','CadClientes');
end;

procedure TFrameListProdutos.UniFrameCreate(Sender: TObject);
begin
 PControl := TPageControl.Init(MainForm.pgcControl);
end;

initialization
  RegisterClass(TFrameListProdutos);
end.
